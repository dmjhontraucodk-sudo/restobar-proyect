...
export const useTeamManagement = () => {
    ...
    const puedeVerSalarios = user?.role === 'Administrador' || user?.role === 'Gerente';
    ...
    const loadEmpleados = useCallback(async () => {
        ...
        try {
            const data = await makeRequest<{ success: boolean; empleados: ApiEmpleado[] }>('/empleados');
            setEmpleados(data.empleados);
        } catch (err: any) {
            console.error('Error al cargar empleados:', err);
            const errorMessage = err.message || 'No se pudieron cargar los empleados.';
    ...
    const loadRoles = useCallback(async () => {
        try {
            const data = await makeRequest<{ success: boolean; roles: ApiRol[] }>('/roles');
            setRoles(data.roles);
        } catch (err: any) {
    ...
    const loadTodosRoles = useCallback(async () => {
        ...
        try {
            const data = await makeRequest<{ success: boolean; roles: ApiRol[] }>('/roles/todos');
    ...
    const createEmpleado = useCallback(async (data: CreateEmpleadoData): Promise<boolean> => {
        try {
            const response = await makeRequest<{ success: boolean; empleado: ApiEmpleado; message: string }>('/empleados', {
                method: 'POST',
    ...
    const updateEmpleado = useCallback(async (id: number, data: UpdateEmpleadoData): Promise<boolean> => {
        try {
            const response = await makeRequest<{ success: boolean; empleado: ApiEmpleado; message: string }>(`/empleados/${id}`, {
                method: 'PATCH',
                body: JSON.stringify(data),
            });
    ...
    const desactivarEmpleado = useCallback(async (id: number): Promise<boolean> => {
        if (!confirm('¿Estás seguro de que deseas desactivar este empleado?')) {
            return false;
        }

        try {
            const response = await makeRequest<{ success: boolean; message: string }>(`/empleados/${id}`, {
                method: 'DELETE',
            });
    ...
    const activarEmpleado = useCallback(async (id: number): Promise<boolean> => {
        try {
            const response = await makeRequest<{ success: boolean; message: string }>(`/empleados/${id}/activar`, {
                method: 'POST',
            });
    ...
    const resetearPassword = useCallback(async (id: number): Promise<string | null> => {
        if (!confirm('¿Resetear la contraseña de este empleado? Se generará una contraseña temporal.')) {
            return null;
        }

        try {
            const response = await makeRequest<ResetPasswordResponse>(`/empleados/${id}/resetear-password`, {
                method: 'POST',
            });
    ...
    const createRol = useCallback(async (data: CreateRolData): Promise<boolean> => {
        ...
        try {
            const response = await makeRequest<{ success: boolean; rol: ApiRol; message: string }>('/roles/crear', {
                method: 'POST',
                body: JSON.stringify(data),
            });
    ...
    const updateRol = useCallback(async (id: number, data: UpdateRolData): Promise<boolean> => {
        if (!puedeGestionarRoles) {
        ...
        try {
            const response = await makeRequest<{ success: boolean; rol: ApiRol; message: string }>(`/roles/${id}`, {
                method: 'PATCH',
                body: JSON.stringify(data),
            });
    ...
    const desactivarRol = useCallback(async (id: number): Promise<boolean> => {
        if (!puedeGestionarRoles) {
        ...
        try {
            const response = await makeRequest<{ success: boolean; message: string }>(`/roles/${id}`, {
                method: 'DELETE',
            });
    ...
    const activarRol = useCallback(async (id: number): Promise<boolean> => {
        if (!puedeGestionarRoles) {
        ...
        try {
            const response = await makeRequest<{ success: boolean; message: string }>(`/roles/${id}/activar`, {
                method: 'POST',
            });
    ...
    }, [loadEmpleados, loadRoles, loadTodosRoles, loadNomina, puedeGestionarRoles, puedeVerSalarios]);
