...
export const useMenuManagement = (tipo: TipoCategoria = 'COMIDA') => {
  ...
  const filteredCategories = useMemo(() => {
    ...
    if (selectedCategory !== "all") {
      categoriesToFilter = categoriesToFilter.filter(c => c && c.id === selectedCategory);
    }
  ...
  const inactiveItemsCount = useMemo(() => {
    return categories.reduce((count, category) => 
      count + (category.items || []).filter(item => item && !item.disponible).length, 0
    );
  }, [categories]);
  ...
  const handleSaveCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Categoría "${nuevaCategoriaApi.nombre}" añadida correctamente`);
  ...
  const handleSaveItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        toast.success(`"${productData.nombre}" actualizado correctamente`);
      } else {
        // Crear
        ...
        toast.success(`"${productData.nombre}" creado con éxito`);
      }
