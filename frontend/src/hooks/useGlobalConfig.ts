...
export const useGlobalConfig = () => {
    ...
    const loadPublicConfig = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('✅ Configuración pública cargada:', data.config);
        } else {
  ...
  useEffect(() => {
    const loadConfig = async () => {
      ...
      try {
        ...
        const response = await fetch('http://localhost:3000/api/dashboard/config', {
          headers: {
  ...
  const verificarHorarioAtencion = () => {
    ...
    const dentroDeHorario = horaActual >= horarioInicio && horaActual <= horarioFin;
  ...
  const getPedidosWebConfig = () => {
    ...
    return {
      ...
      activos: publicConfig?.pedidos_activos ?? config?.pedidos_online_activos ?? true,
      dentroDeHorario: horarioCheck.dentroDeHorario,
      horario: horarioCheck.horario,
      horaActual: horarioCheck.horaActual,
      
      // Costos y montos
      costoDelivery: publicConfig?.costo_delivery ?? config?.costo_delivery ?? 0,
      montoMinimo: publicConfig?.monto_minimo ?? config?.monto_minimo_pedido ?? 0,
      tiempoPrep: publicConfig?.tiempo_preparacion ?? config?.tiempo_prep_web ?? 30,
      
      // Información adicional
      mensajeBienvenida: publicConfig?.mensaje_bienvenida ?? config?.mensaje_bienvenida_web,
      reservasActivas: publicConfig?.reservas_activas ?? config?.reservas_activas ?? false,
      diasLimiteReserva: publicConfig?.dias_limite_reserva ?? config?.dias_limite_reserva ?? 7
    };
  };
    ...
    validaciones: {
      ...
      pedidoValido: (montoActual: number) => {
        ...
        if (montoActual < pedidosConfig.montoMinimo) {
          return {
            disponible: false,
            mensaje: `El monto mínimo de pedido es S/ ${pedidosConfig.montoMinimo.toFixed(2)}`,
