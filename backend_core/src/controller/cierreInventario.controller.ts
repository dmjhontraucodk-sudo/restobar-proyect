...
export const getCierresInventario = async (req: Request, res: Response) => {
  try {
    ...
    if (fechaInicio || fechaFin) {
      where.fecha_inicio = {};
      if (fechaInicio) {
        where.fecha_inicio.gte = new Date(fechaInicio as string);
      }
      if (fechaFin) {
        where.fecha_inicio.lte = new Date(fechaFin as string);
      }
    }
...
// ========== CREAR NUEVO CIERRE ==========
export const createCierreInventario = async (req: Request, res: Response) => {
  try {
    const tenantId = (req as any).tenantId;
...
export const updateCierreInventario = async (req: Request, res: Response) => {
  try {
    const tenantId = (req as any).tenantId;
    const { id } = req.params;
...
export const getCierreEstadisticas = async (req: Request, res: Response) => {
  try {
    const tenantId = (req as any).tenantId;
    const { id } = req.params;
    ...
    const diferenciasPositivas = cierre.detalles.filter(d => 
      new Prisma.Decimal(d.diferencia).greaterThan(0)
    ).length;
    const diferenciasNegativas = cierre.detalles.filter(d => 
      new Prisma.Decimal(d.diferencia).lessThan(0)
    ).length;
