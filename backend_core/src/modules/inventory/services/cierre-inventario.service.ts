...
export const cierreInventarioService = {
    
    async getAll(tenantId: number, filters: any) {
        const where: any = { tenant_id: tenantId };
        if (filters.estado) where.estado = filters.estado;
        if (filters.tipo_cierre) where.tipo_cierre = filters.tipo_cierre;
        if (filters.fechaInicio || filters.fechaFin) {
            where.fecha_inicio = {};
            if (filters.fechaInicio) where.fecha_inicio.gte = filters.fechaInicio;
            if (filters.fechaFin) where.fecha_inicio.lte = filters.fechaFin;
        }

        return await prisma.cierres_inventario.findMany({
            where,
            include: {
    ...
    async getById(tenantId: number, id: number) {
        return await prisma.cierres_inventario.findFirst({
    ...
    async create(tenantId: number, userId: number, data: any) {
        ...
        let totalDiferencias = new Prisma.Decimal(0);
        ...
        for (const detalle of detalles) {
            ...
            if (!producto) throw new Error(`Producto ${detalle.producto_inventario_id} no encontrado`);

            const stockSistema = new Prisma.Decimal(producto.stock_actual || 0);
            const stockFisico = new Prisma.Decimal(detalle.stock_fisico);
            const diferencia = stockFisico.minus(stockSistema);
            const costoUnitario = new Prisma.Decimal(producto.costo_unitario || 0);
        ...
        return await prisma.cierres_inventario.create({
            data: {
                tenant_id: tenantId,
    ...
    async update(tenantId: number, id: number, data: any) {
        ...
        if (existing.estado !== 'Borrador') throw new Error('Solo se pueden editar cierres en Borrador');
        ...
        if (detalles && detalles.length > 0) {
            // Recalcular todo
            await prisma.cierres_inventario_detalles.deleteMany({ where: { cierre_id: id } });
        ...
        return await prisma.cierres_inventario.update({
            where: { id },
            data: updateData,
            include: { detalles: true }
        });
    ...
    async finalizar(tenantId: number, id: number) {
        const cierre = await this.getById(tenantId, id);
        if (!cierre) throw new Error('Cierre no encontrado');
        if (cierre.estado === 'Finalizado') throw new Error('Cierre ya finalizado');
