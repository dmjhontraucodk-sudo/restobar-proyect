...
export interface UpdateCompraData {
  ...
  items?: Array<{
    id?: number; // Para items existentes a actualizar
    producto_inventario_id: number;
    cantidad: number;
    costo_unitario: number;
  }>;
...
export const comprasService = {
  async getAll(tenantId: number, filters: any = {}) {
    ...
    if (filters.tipo_gasto_id) where.tipo_gasto_id = parseInt(filters.tipo_gasto_id);
    if (filters.proveedor_id) where.proveedor_id = parseInt(filters.proveedor_id);
    if (filters.estado_compra) where.estado_compra = filters.estado_compra;
  ...
  async getById(tenantId: number, id: number) {
    return await prisma.compras.findFirst({
  ...
  async create(tenantId: number, data: CreateCompraData) {
    const { items, ...compraData } = data;
    let total = new Prisma.Decimal(0);

    // Validar que tipo_gasto_id y proveedor_id (si existe) pertenezcan al tenant
    const tipoGasto = await prisma.tipos_gasto.findFirst({ where: { id: compraData.tipo_gasto_id, tenant_id: tenantId } });
    if (!tipoGasto) throw new Error('Tipo de gasto inválido.');
    if (compraData.proveedor_id) {
        const proveedor = await prisma.proveedores.findFirst({ where: { id: compraData.proveedor_id, tenant_id: tenantId } });
        if (!proveedor) throw new Error('Proveedor inválido.');
    }

    for (const item of items) {
      const productoInventario = await prisma.productos_inventario.findFirst({ where: { id: item.producto_inventario_id, tenant_id: tenantId } });
      if (!productoInventario) throw new Error(`Producto de inventario con ID ${item.producto_inventario_id} no encontrado.`);
      
      total = total.plus(new Prisma.Decimal(item.cantidad).times(new Prisma.Decimal(item.costo_unitario)));
    }
  ...
  async update(tenantId: number, id: number, data: UpdateCompraData) {
    const existingCompra = await this.getById(tenantId, id);
    if (!existingCompra) throw new Error('Compra no encontrada.');
    ...
    if (items) {
      await prisma.$transaction(async (tx) => {
        ...
        total = existingCompra.compras_detalles.reduce((sum, item) => sum.plus(new Prisma.Decimal(item.cantidad).times(new Prisma.Decimal(item.costo_unitario))), new Prisma.Decimal(0));
    }
  ...
  async delete(tenantId: number, id: number) {
    ...
    return await prisma.$transaction([
      prisma.compras_detalles.deleteMany({ where: { compra_id: id } }),
      prisma.compras.delete({ where: { id } })
    ]);
  },

  async receiveCompra(tenantId: number, id: number) {
    const compra = await this.getById(tenantId, id);
    if (!compra) throw new Error('Compra no encontrada.');
    if (compra.estado_compra === 'Recibida') throw new Error('La compra ya ha sido recibida.');
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      for (const detalle of compra.compras_detalles) {
        ...
        if (!producto) throw new Error(`Producto de inventario con ID ${detalle.producto_inventario_id} no encontrado.`);

        const newStock = new Prisma.Decimal(producto.stock_actual || 0).plus(detalle.cantidad);
        ...
        await tx.kardex.create({
          data: {
            ...
            valor_total: new Prisma.Decimal(detalle.cantidad).times(detalle.costo_unitario),
