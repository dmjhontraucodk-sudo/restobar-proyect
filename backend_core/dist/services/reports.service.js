...
exports.reportsService = {
    ...
    async getSalesSummaryByDateRange(tenantId, range) {
        ...
        const salesData = await prisma_1.prisma.ordenes.aggregate({
        ...
        const webSalesData = await prisma_1.prisma.webpedidos.aggregate({
            _sum: { total: true },
            _count: { id: true },
            where: {
                tenant_id: tenantId,
        ...
        const dailyTrend = await prisma_1.prisma.$queryRaw `
            SELECT 
    ...
    async getInventorySummaryAndAlerts(tenantId, range) {
        ...
        const lowStockCount = await prisma_1.prisma.productos_inventario.count({
            where: {
                tenant_id: tenantId,
                activo: true,
                stock_actual: { lte: prisma_1.prisma.productos_inventario.fields.stock_minimo }
            }
        });
        ...
        const inventoryValueResult = await prisma_1.prisma.$queryRaw `
        ...
        const mermaValueResult = await prisma_1.prisma.cierres_inventario.aggregate({
        ...
        const comprasTotal = await prisma_1.prisma.compras.aggregate({
            _sum: { total: true },
            where: {
                tenant_id: tenantId,
    ...
    async getFinanceSummaryByDateRange(tenantId, range) {
        ...
        const pagosPorMetodo = await prisma_1.prisma.pagos.groupBy({
            by: ['metodo_pago'],
            _sum: { monto: true },
            where: {
                tenant_id: tenantId,
        ...
        const gastosOperativos = await prisma_1.prisma.gastos.findMany({
            where: {
                tenant_id: tenantId,
                fecha: { gte: startDate, lte: endDate }
            },
        ...
        const cierresCaja = await prisma_1.prisma.cajas.findMany({
            where: {
                tenant_id: tenantId,
                estado: 'Cerrada',
