...
// backend_core/src/services/roles.service.ts - CORREGIDO PARA MYSQL
...
exports.rolesService = {
    ...
    async getRolById(rolId) {
        return prisma_1.prisma.roles.findUnique({
            where: { id: rolId }
        });
    },
    /**
    ...
    async contarEmpleadosPorRol(rolId, tenantId) {
        return prisma_1.prisma.empleados.count({
            where: {
                rol_id: rolId,
                tenant_id: tenantId
            }
    ...
    async createRol(data) {
        ...
        const existingRol = await prisma_1.prisma.roles.findFirst({
            where: {
                nombre: data.nombre
            }
        });
        ...
        // Crear rol
        return prisma_1.prisma.roles.create({
            data: {
                nombre: data.nombre,
    ...
    async updateRol(rolId, data) {
        ...
        // Actualizar rol
        return prisma_1.prisma.roles.update({
            where: { id: rolId },
            data: {
                nombre: data.nombre,
    ...
    async desactivarRol(rolId) {
        ...
        const empleadosActivos = await prisma_1.prisma.empleados.count({
            where: {
                rol_id: rolId,
                is_active: true
            }
