...
exports.inventoryController = {
    ...
    async verificarStockBajo(req, res) {
        try {
            ...
            const productosStockBajo = await inventory_alerts_service_1.inventoryAlertsService.verificarStockBajo(tenantId);
            res.json({
                success: true,
     ...
     */
    async getProductosStockCritico(req, res) {
        try {
            const tenantId = req.user?.tenant_id;
            if (!tenantId) {
                return res.status(403).json({ error: 'Acceso no autorizado' });
            ...
            const productosAgotados = await prisma_1.prisma.productos_inventario.findMany({
                where: {
                    tenant_id: tenantId,
                    stock_actual: {
                        lte: 0
    ...
    async verificarDisponibilidad(req, res) {
        try {
            ...
            const resultado = await inventory_alerts_service_1.inventoryAlertsService.verificarDisponibilidadPedido(tenantId, items);
    ...
    async actualizarStock(req, res) {
        try {
            ...
            const productoInventario = await prisma_1.prisma.productos_inventario.findUnique({
            ...
            // Actualizar stock
            const productoActualizado = await prisma_1.prisma.productos_inventario.update({
                where: { id: productoInvId },
                data: {
            ...
            // Verificar alertas
            if (stockNuevo <= 0) {
                await inventory_alerts_service_1.inventoryAlertsService.verificarProductoAgotado(tenantId, productoInvId, productoInventario.nombre, stockNuevo);
            }
            else {
