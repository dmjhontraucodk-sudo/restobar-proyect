...
exports.inventoryAlertsService = {
    ...
    async verificarStockBajo(tenantId) {
        try {
            ...
            const productosStockBajo = await prisma_service_1.prisma.productos_inventario.findMany({
                where: {
                    tenant_id: tenantId,
                    stock_actual: {
                        lte: nivelAlerta,
    ...
    async verificarDisponibilidadPedido(tenantId, items) {
        ...
        for (const item of items) {
            ...
            const productoMenu = await prisma_service_1.prisma.productos.findUnique({
