...
exports.webOrdersService = {
    async getWebOrdersByTenant(tenantId, filters) {
        ...
        return await prisma_service_1.prisma.webpedidos.findMany({
            where,
            include: {
                webpedidos_detalles: {
                    include: {
    ...
    async processInventoryDeduction(tenantId, orderId, userId) {
        const order = await prisma_service_1.prisma.webpedidos.findUnique({
            where: { id: orderId },
            include: {
                webpedidos_detalles: {
                    include: {
    ...
    async getWebOrderById(tenantId, orderId) {
        return await prisma_service_1.prisma.webpedidos.findFirst({
            where: {
                id: orderId,
                tenant_id: tenantId
            },
    ...
    async createWebOrder(tenantId, orderData) {
        ...
        const productos = await prisma_service_1.prisma.productos.findMany({
            where: {
                id: { in: productIds },
                tenant_id: tenantId,
                disponible: true
        ...
        const tenantConfig = await prisma_service_1.prisma.tenant_config.findUnique({
        ...
        return await prisma_service_1.prisma.webpedidos.create({
            data: {
                tenant_id: tenantId,
                cliente_nombre,
    ...
    async updateOrderStatus(tenantId, orderId, newStatus) {
        ...
        const existingOrder = await prisma_service_1.prisma.webpedidos.findFirst({
            where: {
                id: orderId,
                tenant_id: tenantId
            }
        ...
        return await prisma_service_1.prisma.webpedidos.update({
            where: {
                id: orderId
            },
            data: {
    ...
    async convertToPosOrder(tenantId, webOrderId, empleadoId) {
        ...
        const posOrder = await prisma_service_1.prisma.ordenes.create({
            data: {
                tenant_id: tenantId,
                mesa_id: 1,
    ...
    async getOrderConfig(tenantId) {
        let config = await prisma_service_1.prisma.tenant_config_pedidos.findUnique({
            where: { tenant_id: tenantId }
        });
        if (!config) {
            config = await prisma_service_1.prisma.tenant_config_pedidos.create({
                data: {
                    tenant_id: tenantId,
    ...
    async updateOrderConfig(tenantId, configData) {
        const existingConfig = await prisma_service_1.prisma.tenant_config_pedidos.findUnique({
            where: { tenant_id: tenantId }
        });
        if (existingConfig) {
            return await prisma_service_1.prisma.tenant_config_pedidos.update({
                where: { tenant_id: tenantId },
                data: configData
            });
        }
        else {
            return await prisma_service_1.prisma.tenant_config_pedidos.create({
                data: {
                    tenant_id: tenantId,
                    ...configData
                }
    ...
    async getOrderStats(tenantId) {
        ...
        const [totalPedidos, pedidosHoy, pedidosPendientes, pedidosEnPreparacion, ventasHoy] = await Promise.all([
            prisma_service_1.prisma.webpedidos.count({
                where: { tenant_id: tenantId }
            }),
            prisma_service_1.prisma.webpedidos.count({
                where: {
            ...
            prisma_service_1.prisma.webpedidos.aggregate({
                where: {
                    tenant_id: tenantId,
                    created_at: {
                        gte: today,
    ...
    async getOrderStatsByStatus(tenantId) {
        const stats = await prisma_service_1.prisma.webpedidos.groupBy({
            by: ['estado'],
            where: { tenant_id: tenantId },
            _count: {
                id: true
