...
exports.empleadosService = {
    ...
    async getAllEmpleados(tenantId) {
        return prisma_service_1.prisma.empleados.findMany({
    ...
    async getEmpleadoById(tenantId, empleadoId) {
        return prisma_service_1.prisma.empleados.findFirst({
            where: {
                id: empleadoId,
                tenant_id: tenantId
            },
    ...
    async createEmpleado(tenantId, data) {
        ...
        const existingEmail = await prisma_service_1.prisma.empleados.findFirst({
            where: {
                tenant_id: tenantId,
                email: data.email
            }
        ...
        const rol = await prisma_service_1.prisma.roles.findUnique({
        ...
        const fechaIngresoDate = data.fecha_ingreso
            ...
            : null;
        // Crear empleado
        const nuevoEmpleado = await prisma_service_1.prisma.empleados.create({
            data: {
                tenant_id: tenantId,
                nombre: data.nombre,
                email: data.email,
    ...
    async updateEmpleado(tenantId, empleadoId, data) {
        ...
        const fechaIngresoDate = data.fecha_ingreso !== undefined
            ...
            : undefined;
        return prisma_service_1.prisma.empleados.update({
            where: { id: empleadoId },
            data: {
                nombre: data.nombre,
                email: data.email,
    ...
    async cambiarPassword(tenantId, empleadoId, nuevaPassword) {
        ...
        await prisma_service_1.prisma.empleados.update({
            where: { id: empleadoId },
            data: {
                password_hash: passwordHash,
