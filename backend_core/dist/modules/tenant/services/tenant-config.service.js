...
exports.tenantConfigService = {
    ...
    async updateOrderConfig(tenantId, configData) {
        return await prisma_service_1.prisma.tenant_config_pedidos.upsert({
            where: { tenant_id: tenantId },
            update: configData,
            create: {
                tenant_id: tenantId,
    ...
    async getConfig(tenantId) {
        let config = await prisma_service_1.prisma.tenant_config.findUnique({
            where: { tenant_id: tenantId }
        });
    ...
    async createDefaultConfig(tenantId) {
        return await prisma_service_1.prisma.tenant_config.create({
            data: {
                tenant_id: tenantId,
    ...
    async updateConfig(tenantId, data) {
        const existingConfig = await prisma_service_1.prisma.tenant_config.findUnique({
            where: { tenant_id: tenantId }
        });
        if (!existingConfig) {
        ...
        return await prisma_service_1.prisma.tenant_config.update({
            where: { tenant_id: tenantId },
            data
        });
    },
     ...
     */
    async updateSection(tenantId, _section, data) {
