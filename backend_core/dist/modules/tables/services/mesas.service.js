...
exports.mesasService = {
    ...
    async getAllMesas(tenantId) {
        return prisma_service_1.prisma.mesas.findMany({
            where: { tenant_id: tenantId },
            orderBy: { nombre_o_numero: 'asc' },
        });
    },
    async getMesaById(tenantId, mesaId) {
        return prisma_service_1.prisma.mesas.findUnique({
            where: { id: mesaId, tenant_id: tenantId },
        });
    },
    // --- CREAR ---
    async createMesa(tenantId, data) {
        const existing = await prisma_service_1.prisma.mesas.findFirst({
        ...
        return prisma_service_1.prisma.mesas.create({
            data: {
    ...
    async updateMesa(tenantId, mesaId, data) {
        return prisma_service_1.prisma.mesas.update({
            where: { id: mesaId, tenant_id: tenantId },
            data: data,
        }).catch((err) => {
            if (err.code === 'P2025') {
    ...
    async deleteMesa(tenantId, mesaId) {
        const mesa = await prisma_service_1.prisma.mesas.findUnique({
            where: { id: mesaId, tenant_id: tenantId }
        });
        if (!mesa) {
            throw new Error('Mesa no encontrada.');
        ...
        return prisma_service_1.prisma.mesas.delete({
            where: { id: mesaId, tenant_id: tenantId },
        });
    },
    // --- CAMBIAR ESTADO ---
    ...
    async getAvailableMesas(tenantId) {
        ...
        return await prisma_service_1.prisma.mesas.findMany({
            where: {
                tenant_id: tenantId,
                estado: 'Libre'
            },
